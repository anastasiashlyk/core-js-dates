...
function getTime(date) {
  return date.toTimeString().split(' ').slice(0, 1);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate1 <= endDate) {
    // If within the working part of the cycle, add the date
    if (dayIndex < countWorkDays) {
      result.push(formatDate1(currentDate1));
    }
